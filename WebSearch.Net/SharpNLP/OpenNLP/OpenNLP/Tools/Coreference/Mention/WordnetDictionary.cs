...
{
	/// <summary> 
    /// An implementation of the Dictionary interface using the SharpWordnet library.
    /// </summary>
	public class WordnetDictionary : IDictionary
	{
        private WordNetEngine _engine;
        private IOperation[] _defaultOperations;
		...
		{
            _engine = new DataFileEngine(searchDirectory);
            Dictionary<string, string[][]> suffixMap = new Dictionary<string, string[][]>();
            suffixMap.Add("noun", new string[][] { new string[] { "s", "" }, 
                new string[] { "ses", "s" }, new string[] { "xes", "x" }, 
                new string[] { "zes", "z" }, new string[] { "ches", "ch" }, 
                new string[] { "shes", "sh" }, new string[] { "men", "man" },
                new string[] { "ies", "y" } });
            suffixMap.Add("verb", new string[][] { new string[] { "s", "" }, 
                new string[] { "ies", "y" }, new string[] { "es", "e" }, 
                new string[] { "es", "" }, new string[] { "ed", "e" }, 
                new string[] { "ed", "" }, new string[] { "ing", "e" }, 
                new string[] { "ing", "" } });
            suffixMap.Add("adjective", new string[][] { new string[] { "er", "" },
                new string[] { "est", "" }, new string[] { "er", "e" },
                new string[] { "est", "e" } });
            DetachSuffixesOperation tokDso = new DetachSuffixesOperation(suffixMap);
            tokDso.AddDelegate(DetachSuffixesOperation.Operations, new IOperation[]
            { new LookupIndexWordOperation(_engine), new LookupExceptionsOperation(_engine) });
            TokenizerOperation tokOp = new TokenizerOperation(_engine, new string[] { " ", "-" });
            tokOp.AddDelegate(TokenizerOperation.TokenOperations, new IOperation[] 
            { new LookupIndexWordOperation(_engine), new LookupExceptionsOperation(_engine), tokDso });
            DetachSuffixesOperation morphDso = new DetachSuffixesOperation(suffixMap);
            morphDso.AddDelegate(DetachSuffixesOperation.Operations, new IOperation[]
            { new LookupIndexWordOperation(_engine), new LookupExceptionsOperation(_engine) });
            _defaultOperations = new IOperation[] { new LookupExceptionsOperation(_engine), morphDso, tokOp };
		}
		...
		public virtual string[] GetLemmas(string word, string tag)
		{
            ...
            return _engine.GetBaseForms(word, partOfSpeech, _defaultOperations);
		}
		
		public virtual string GetSenseKey(string lemma, string partOfSpeech, int sense)
		{
            IndexWord indexWord = _engine.GetIndexWord(lemma, "noun");
            if (indexWord == null)
            {
                return null;
            }
        ...
        {
            Synset[] synsets = _engine.GetSynsets(lemma, "noun");
