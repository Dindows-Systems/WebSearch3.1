...
namespace OpenNLP.Tools.Coreference.Similarity
{
	...
	public class GenderModel : ITestGenderModel, ITrainSimilarityModel
	{
        ...
        private GenderModel(string modelName, bool train)
        {
            ...
            {
                mTestModel = new SharpEntropy.GisModel(new SharpEntropy.IO.
                    BinaryGisModelReader(modelName + mModelExtension));
		...
		private List<string> GetFeatures(Context nounPhrase)
		{
            ...
            if (nounPhrase.NameType != null && nounPhrase.NameType == "person")
			{
				...
				for (int tokenIndex = 0; tokenIndex < nounPhrase.HeadTokenIndex ||
		...
		public static string GenderMain(string modelName, string line)
		{
            ...
            Context context = Context.ParseContext(words[0]);
            if (context == null) return null;

			double[] dist = model.GenderDistribution(context);
			string output = "m=" + dist[model.MaleIndex] + " f=" + dist[model.FemaleIndex] + 
                " n=" + dist[model.NeuterIndex] + " " + string.Join(",", 
                (model.GetFeatures(Context.ParseContext(words[0])).ToArray()));
            return output;
		}
