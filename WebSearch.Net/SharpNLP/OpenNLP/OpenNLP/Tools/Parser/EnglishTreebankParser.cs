...
namespace OpenNLP.Tools.Parser
{
    ...
    public sealed class EnglishTreebankParser
    {
        ...
        public EnglishTreebankParser(string dataDirectory, bool useTagDictionary,
            bool useCaseSensitiveTagDictionary, int beamSize, double advancePercentage)
        {
            BinaryGisModelReader buildModelReader = new BinaryGisModelReader(
                dataDirectory + "parser\\build.nbin");
            GisModel buildModel = new GisModel(buildModelReader);

            BinaryGisModelReader checkModelReader = new BinaryGisModelReader(
                dataDirectory + "parser\\check.nbin");
            IMaximumEntropyModel checkModel = new GisModel(checkModelReader);

            EnglishTreebankPosTagger posTagger;

            if (useTagDictionary)
                posTagger = new EnglishTreebankPosTagger(dataDirectory +
                    "parser\\tag.nbin", dataDirectory + "parser\\tagdict",
                    useCaseSensitiveTagDictionary);
            else
                posTagger = new EnglishTreebankPosTagger(dataDirectory + "parser\\tag.nbin");

            EnglishTreebankParserChunker chunker = new
                EnglishTreebankParserChunker(dataDirectory + "parser\\chunk.nbin");
            EnglishHeadRules headRules = new EnglishHeadRules(dataDirectory + "parser\\head_rules");

            mParser = new MaximumEntropyParser(buildModel, checkModel,
                posTagger, chunker, headRules, beamSize, advancePercentage);

            mTokenizer = new EnglishMaximumEntropyTokenizer(dataDirectory + "EnglishTok.nbin");

        }

        public EnglishTreebankParser(string dataDirectory)
            : this(dataDirectory, true, false, MaximumEntropyParser.DefaultBeamSize,
            MaximumEntropyParser.DefaultAdvancePercentage)
        {
        }

        public EnglishTreebankParser(string dataDirectory, bool useTagDictionary,
            bool useCaseSensitiveTagDictionary)
            : this(dataDirectory,
            useTagDictionary, useCaseSensitiveTagDictionary,
            MaximumEntropyParser.DefaultBeamSize,
            MaximumEntropyParser.DefaultAdvancePercentage)
        {
        }

        public EnglishTreebankParser(string dataDirectory, bool useTagDictionary,
            bool useCaseSensitiveTagDictionary, int beamSize)
            : this(dataDirectory, useTagDictionary, useCaseSensitiveTagDictionary,
            beamSize, MaximumEntropyParser.DefaultAdvancePercentage)
        {
        }

        public EnglishTreebankParser(string dataDirectory, bool useTagDictionary,
            bool useCaseSensitiveTagDictionary, double advancePercentage)
            : this(dataDirectory, useTagDictionary, useCaseSensitiveTagDictionary,
            MaximumEntropyParser.DefaultBeamSize, advancePercentage)
        {
        }
        ...
        private class EnglishTreebankPosTagger : PosTagger.MaximumEntropyPosTagger, IParserTagger
        {
            ...
            public EnglishTreebankPosTagger(string modelFile)
                : this(modelFile, K, K)
            {
            }

            public EnglishTreebankPosTagger(string modelFile,
                int beamSize, int cacheSize)
                : base(beamSize,
                new SharpEntropy.GisModel(new BinaryGisModelReader(modelFile)),
                new PosTagger.DefaultPosContextGenerator(cacheSize), null)
            {
                mBeamSize = beamSize;
            }

            public EnglishTreebankPosTagger(string modelFile,
                string tagDictionary, bool useCase)
                :
            ...
            public EnglishTreebankPosTagger(string modelFile,
                int beamSize, string tagDictionary, bool useCase,
                int cacheSize)
                : base(beamSize, new GisModel(
                new BinaryGisModelReader(modelFile)),
                new DefaultPosContextGenerator(cacheSize),
                new PosTagger.PosLookupList(tagDictionary, useCase))
            {
                mBeamSize = beamSize;
            }
        ...
        private class EnglishTreebankParserChunker : MaximumEntropyChunker, IParserChunker
        {
            private const int K = 10;
            private int mBeamSize;
            private Dictionary<string, string> mContinueStartMap;

            public EnglishTreebankParserChunker(string modelFile)
                : this(modelFile, K, K)
            {
            }
            ...
            public EnglishTreebankParserChunker(string modelFile,
                ...
                : base(new GisModel(
                new BinaryGisModelReader(modelFile)),
                new ChunkContextGenerator(cacheSize), beamSize)
            {
                mContinueStartMap = new Dictionary<string, string>(Model.OutcomeCount);
                for (int currentOutcome = 0, outcomeCount = Model.OutcomeCount;
                    currentOutcome < outcomeCount; currentOutcome++)
                {
                    ...
                    {
                        mContinueStartMap.Add(outcome,
                            MaximumEntropyParser.StartPrefix +
                            outcome.Substring(MaximumEntropyParser.ContinuePrefix.Length));
                    }
                }
                mBeamSize = beamSize;
            }
            ...
            public virtual Util.Sequence[] TopKSequences(string[] sentence,
                string[] tags, double minSequenceScore)
            {
                return Beam.BestSequences(mBeamSize, sentence,
                    new object[] { tags }, minSequenceScore);
            }
        ...
        public string DoParse(string[] lines, int requestedParses)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (lineBuilder.Length != 0)
                {
                    ...
                    {
                        currentParse.Insert(new Parse(text,
                            new Util.Span(start, start + token.Length),
                            MaximumEntropyParser.TokenNode, 0));
                        start += token.Length + 1;
                    }

                    Parse[] parses = mParser.FullParse(currentParse, requestedParses);
                    for (int currentParseIndex = 0, parseCount = parses.Length;
                        currentParseIndex < parseCount; currentParseIndex++)
                    {
                        if (requestedParses > 1)
                        {
                            lineBuilder.Append(currentParse.ToString() +
                                " " + parses[currentParseIndex].Probability.ToString(
        ...
        public Parse[] DoParse(string line, int requestedParses)
        {
            ...
            if (lineBuilder.Length != 0)
            {
                ...
                foreach (string token in tokens)
                {
                    currentParse.Insert(new Parse(text, new Util.Span(start,
                        start + token.Length), MaximumEntropyParser.TokenNode, 0));
                    start += token.Length + 1;
                }
