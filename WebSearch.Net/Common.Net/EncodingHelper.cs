...
namespace WebSearch.Common.Net
{
    public class EncodingHelper
    {
        ...
        public static string GetEncodingName(System.Uri testurl)
        {
            try
            {
                HttpWebResponse response = (HttpWebResponse)WebRequest.
                    Create(testurl.AbsoluteUri).GetResponse();
                string encodingName = GetEncodingName(response);
                response.Close();
                return encodingName;
            }
            catch (System.Exception e)
            {
                throw new Exception("Error loading or using URL " + e.ToString());
            }
        }

        public static string GetEncodingName(WebResponse response)
        {
            byte[] rawtext = new byte[1024];
            int bytesread = 0, byteoffset = 0;

            Stream stream = response.GetResponseStream();
            while ((bytesread = ReadInput(stream, ref rawtext,
                byteoffset, rawtext.Length - byteoffset)) > 0)
                byteoffset += bytesread;

            stream.Close();

            // first, try to get the encoding from web response headers
            string name1 = GetEncodingName(response.Headers);
            // second, try the meta section
            string name2 = GetEncodingNameFromMeta(rawtext);
            // if failed, analyze the stream byte by byte
            string name3 = GetEncodingName(rawtext);

            return GetMostFrequent(name1, name2, name3);
        }

        public static Encoding GetEncoding(WebResponse response, out string content)
        {
            content = "";
            ...
            EncodingHelper.ReadInput(stream, ref completebytes);
            stream.Close();

            // first, try to get the encoding from web response headers
            string name1 = GetEncodingName(response.Headers);
            // second, try to get the encoding from meta data
            string name2 = GetEncodingNameFromMeta(completebytes);
            // third, try to get the encoding from content in byte
            string name3 = GetEncodingName(completebytes);

            string name = GetMostFrequent(name1, name2, name3);
            if (name == "") return null;
           
            Encoding encoding = Encoding.GetEncoding(name);
            content = encoding.GetString(completebytes);
            return encoding;
        }
        ...
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", 
        /// "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        /// <example>
        ...
        /// </code>
        /// </example>
        public static string GetEncodingName(FileInfo testfile)
        {
            System.IO.FileStream chinesefile;
            byte[] rawtext;
            rawtext = new byte[(int)FileLength(testfile)];
            try
            {
                chinesefile = new System.IO.FileStream(testfile.FullName,
        ...
        /// <summary>
        /// 从指定的 <see cref="byte"/> 字节数组中判断编码类型
        /// </summary>
        ...
        /// <code>
        ///  IdentifyEncoding ide = new IdentifyEncoding();
        ///  Response.Write(ide.GetEncodingName(IdentifyEncoding.TobyteArray
        /// (System.Text.Encoding.GetEncoding("gb2312").GetBytes("Lion互动网络(www.lionsky.net)")))); 
        /// </code>
        /// </example>
        public static string GetEncodingName(byte[] rawtext)
        {
            int[] scores;
        ...
        #region Header Charset

        public static string GetEncodingName(WebHeaderCollection headers)
        {
        ...
        public static string GetEncodingNameFromMeta(byte[] bytes)
        {
            string reg_charset = "(<meta[^>]*charset=(?<charset>[^>'\"]*)" +
                "[\\s\\S]*?>)|(xml[^>]+encoding=(\"|')*(?<charset>[^>'\"]*)[\\s\\S]*?>)";
            Regex r = new Regex(reg_charset, RegexOptions.IgnoreCase);
            string temp = Encoding.GetEncoding("utf-8").GetString(bytes);
            Match m = r.Match(temp);
            return (m.Captures.Count != 0) ? m.Result("${charset}") : "";
        }
        ...
        internal static int GB2312Probability(byte[] rawtext)
        {
            int i, rawtextlen = 0;
            ...
            rawtextlen = (rawtext.Length > 1024) ? 1024 : rawtext.Length;
            for (i = 0; i < rawtextlen - 1; i++)
            {
                ...
                {
                    dbchars++;
                    if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <=
                        (byte)Identity(0xF7) && (byte)Identity(0xA1) <=
                        rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))
                    {
                        gbchars++;
                        totalfreq += 500;
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int GBKProbability(byte[] rawtext)
        {
            int i, rawtextlen = 0;
            ...
            for (i = 0; i < rawtextlen - 1; i++)
            {
                ...
                else
                {
                    ...
                    else if ((byte)Identity(0x81) <= rawtext[i] && rawtext[i]
                        <= (byte)Identity(0xFE) && (((byte)Identity(0x80)
                        <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int HZProbability(byte[] rawtext)
        {
            int i, rawtextlen;
                ...
                if (rawtext[i] == '~')
                {
                    if (rawtext[i + 1] == '{')
                    {
                        ...
                        while (i < rawtextlen - 1)
                        {
                            ...
                            else if (((byte)0xA1 <= rawtext[i] && rawtext[i] <=
                                (byte)0xF7) && ((byte)0xA1 <= rawtext[i + 1] &&
                                rawtext[i + 1] <= (byte)0xF7))
                            {
                                hzchars += 2;
                                row = rawtext[i] + 256 - 0xA1;
                                column = rawtext[i + 1] + 256 - 0xA1;
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int BIG5Probability(byte[] rawtext)
        {
            int i, rawtextlen = 0;
            ...
            for (i = 0; i < rawtextlen - 1; i++)
            {
                ...
                else
                {
                    dbchars++;
                    if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <=
                        (byte)Identity(0xF9) && (((byte)0x40 <= rawtext[i + 1] &&
                        rawtext[i + 1] <= (byte)0x7E) || ((byte)Identity(0xA1) <=
                        rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))))
                    {
                        bfchars++;
                        totalfreq += 500;
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int ENCTWProbability(byte[] rawtext)
        {
            int i, rawtextlen = 0;
            ...
            for (i = 0; i < rawtextlen - 1; i++)
            {
                ...
                else
                {
                    // high bit set
                    dbchars++;
                    if (i + 3 < rawtextlen && (byte)Identity(0x8E) == rawtext[i] &&
                        (byte)Identity(0xA1) <= rawtext[i + 1] && rawtext[i + 1] <=
                        (byte)Identity(0xB0) && (byte)Identity(0xA1) <= rawtext[i + 2] &&
                        rawtext[i + 2] <= (byte)Identity(0xFE) && (byte)Identity(0xA1) <=
                        rawtext[i + 3] && rawtext[i + 3] <= (byte)Identity(0xFE))
                    {
                        // Planes 1 - 16
                    ...
                    else if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <=
                        (byte)Identity(0xFE) && (byte)Identity(0xA1) <=
                        rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))
                    {
                        cnschars++;
                        totalfreq += 500;
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int ISO2022CNProbability(byte[] rawtext)
        {
            int i, rawtextlen = 0;
            ...
            for (i = 0; i < rawtextlen - 1; i++)
            {
                if (rawtext[i] == (byte)0x1B && i + 3 < rawtextlen)
                {
                    ...
                    if (rawtext[i] == (byte)0x1B && i + 2 < rawtextlen &&
                        rawtext[i + 1] == (byte)0x28 && rawtext[i + 2] == (byte)0x42)
                    {
                        // ASCII:  ESC ( B
                        i += 2;
                    }
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int UTF8Probability(byte[] rawtext)
        {
            int score = 0;
            int i, rawtextlen = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int UnicodeProbability(byte[] rawtext)
        {
            //int score = 0;
            //int i, rawtextlen = 0;
            //int goodbytes = 0, asciibytes = 0;

            if (((byte)Identity(0xFE) == rawtext[0] &&
                (byte)Identity(0xFF) == rawtext[1]) ||
                ((byte)Identity(0xFF) == rawtext[0] &&
                (byte)Identity(0xFE) == rawtext[1]))
            {
                return 100;
            }
        ...
        /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal static int ASCIIProbability(byte[] rawtext)
        {
            int score = 70;
            int i, rawtextlen;
        ...
        /// 初始化必要的条件
        /// </summary>
        internal static void Initialize_Frequencies()
        {
            int i;
            if (GBFreq[0] == null)
            {
        ...
        /// <returns></returns>
        public static int ReadInput(System.IO.Stream sourceStream, ref byte[] target)
        {
            byte[] buffer = new byte[0x100];
            MemoryStream memStream = new MemoryStream();
            for (int i = sourceStream.Read(buffer, 0, 0x100);
                i > 0; i = sourceStream.Read(buffer, 0, 0x100))
                memStream.Write(buffer, 0, i);
            target = memStream.ToArray();
            memStream.Close();
        ...
        public static int ReadInput(System.IO.Stream sourceStream,
            ref byte[] target, int start, int count)
        {
        ...
        /// <exception cref="NotSupportedException">流不支持读取。</exception>
        /// <exception cref="ObjectDisposedException">在流关闭后调用方法。</exception>
        public static int ReadInput(System.IO.TextReader sourceTextReader,
            ref byte[] target, int start, int count)
        {
            // Returns 0 bytes if not enough space in target
        ...
        private static string GetMostFrequent(string encodingName1,
            string encodingName2, string encodingName3)
        {
            encodingName1 = encodingName1.ToLower();
            encodingName2 = encodingName2.ToLower();
            encodingName3 = encodingName3.ToLower();

            Hashtable encodingList = new Hashtable(3);
            if (encodingName1 != "" && encodingName1 != "OTHER")
                encodingList.Add(encodingName1, 1);
            if (encodingName2 != "" && encodingName2 != "0THER")
            {
                if (encodingList.Contains(encodingName2))
                    encodingList[encodingName2] =
                        (int)encodingList[encodingName2] + 1;
                else
                    encodingList.Add(encodingName2, 1);
            }
            if (encodingName3 != "" && encodingName3 != "0THER")
            {
                if (encodingList.Contains(encodingName3))
                    encodingList[encodingName2] =
                        (int)encodingList[encodingName2] + 1;
                else
                    encodingList.Add(encodingName3, 1);
            }
            // figure out the most fequent result
            int max = 0; string result = "";
            foreach (string encoding in encodingList.Keys)
            {
                if (((int)encodingList[encoding]) > max)
                {
                    result = encoding;
                    max = (int)encodingList[encoding];
                }
            }
            // give the gb2312 higher priority
            if (encodingList.Contains("gb2312") &&
                ((int)encodingList["gb2312"]) == max)
                return "gb2312";
            return result;
        }
