            ...
            try
            {
                ...
                while ((token = tokenStream.Next()) != null)
                {
                    if ((tokenGroup.numTokens > 0) && (tokenGroup.IsDistinct(token)))
                    {
                        ...
                        tokenText = text.Substring(startOffset,
                            (endOffset) - (startOffset));

                        System.String markedUpText = formatter.HighlightTerm(
                            encoder.EncodeText(tokenText), tokenGroup);
                        //store any whitespace etc from between this and last group
                        if (startOffset > lastEndOffset)
                            newText.Append(encoder.EncodeText(text.Substring(
                                lastEndOffset, (startOffset) - (lastEndOffset))));
                        ...
                        if (textFragmenter.IsNewFragment(token))
                        {
                            ...
                            currentFrag = new TextFragment(
                                newText, newText.Length, docFrags.Count);
                            fragmentScorer.StartFragment(currentFrag);
                            docFrags.Add(currentFrag);
                        }
                    }
